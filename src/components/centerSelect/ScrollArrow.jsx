...
export function ScrollArrow({ isPositioned, dir, scrollRef, scrollTop, onScroll, innerOffset, onHide }) {
  ...
  const handlePointerEnter = () => {
    ...
    function frame() {
      if (scrollRef.current) {
        ...
        const remainingPixels = dir === "up" ? scrollRef.current.scrollTop : scrollRef.current.scrollHeight - scrollRef.current.clientHeight - scrollRef.current.scrollTop;

        const scrollRemaining = dir === "up" ? scrollRef.current.scrollTop - pixelsToScroll > 0 : scrollRef.current.scrollTop + pixelsToScroll < scrollRef.current.scrollHeight - scrollRef.current.clientHeight;
  ...
  return (
    <div
      className="CenterSelect-ScrollArrow"
      data-dir={dir}
      ref={ref}
      aria-hidden
